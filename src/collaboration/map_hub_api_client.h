...
public:
  ...
  struct Error {
    ...
    Error() = default;
    Error(int status, QString error_code, QString error_message,
          QJsonObject error_details = {})
        : http_status{status}, code{std::move(error_code)},
          message{std::move(error_message)}, details{std::move(error_details)} {}
  ...
  void submitRevision(const QString &revision_id,
                      const QString &client_instance_id,
                      const QString &editing_lease,
                      JsonHandler handler);
  void renewLease(const QString &workspace_id,
                  const QString &client_instance_id,
                  const QString &editing_lease,
                  JsonHandler handler);
  void releaseLease(const QString &workspace_id,
                    const QString &client_instance_id,
                    const QString &editing_lease,
                    JsonHandler handler);
  ...
  /** Canonical scheme/host/effective-port origin for an acceptable Map Hub
   * server URL, or an empty string when transport validation fails. */
  static QString canonicalServerOrigin(const QString &server_url);
