      ...
      [this](const QJsonObject &response, const MapHubApiClient::Error &error) {
        ...
        QTimer::singleShot(
            ...
            this, [this] {
              if (running)
                finish({},
                       {0, QStringLiteral("connection_expired"),
...
void MapHubDeviceAuthorization::cancel() {
  if (running)
    finish({}, {0, QStringLiteral("cancelled"),
                tr("Map Hub sign-in was cancelled.")});
}
  ...
  client->exchangeMapperConnection(
      ...
      [this](const QJsonObject &response, const MapHubApiClient::Error &error) {
        ...
        if (status != QLatin1String("connected")) {
          finish({}, invalidResponse(
                         tr("Map Hub returned an invalid sign-in status.")));
          return;
        }
        Result result;
        ...
        if (result.token.isEmpty() || result.token.toUtf8().size() > 4096) {
          finish({}, invalidResponse(
                         tr("Map Hub returned an invalid account credential.")));
          return;
        }
        finish(result, {});
      });
...
void MapHubDeviceAuthorization::finish(
    const Result &result, const MapHubApiClient::Error &error) {
  if (!running)
    return;
  running = false;
  poll_timer->stop();
