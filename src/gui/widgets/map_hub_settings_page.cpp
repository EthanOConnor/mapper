...
MapHubSettingsPage::MapHubSettingsPage(QWidget *parent)
  ...
  auto *passkey_help = new QLabel(
      tr("Connect through your browser with a passkey. Mapper stores the "
         "resulting account credential in this device's credential store and "
         "uses it for Map Hub maps and imagery."),
      this);
  ...
  auto *invitation_help = new QLabel(
      tr("Use this to create an account from an invitation. After setup, "
         "return here and choose Connect with passkey."),
      this);
...
void MapHubSettingsPage::updateCredentialStatus() {
  ...
  if (!result.error.isEmpty())
    ...
    credential_status->setText(
        tr("Connected. The account token is stored in an owner-only Mapper "
           "application file."));
  else
...
void MapHubSettingsPage::connectWithPasskey() {
  ...
  connect(
      ...
      [this, server](const MapHubDeviceAuthorization::Result &result,
        ...
        loaded_server = server;
        credential_status->setText(
            result.organization_name.isEmpty()
                ? tr("Connected to Map Hub.")
