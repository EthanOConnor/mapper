...
public:
  ...
  AbstractHomeScreenWidget(HomeScreenController *controller,
...
protected:
  ...
  QAbstractButton *makeButton(const QString &text,
                              QWidget *parent = nullptr) const;

  /** Returns a button with the given text and icon
   *  for triggering a top level activity in the home screen. */
  QAbstractButton *makeButton(const QString &text, const QIcon &icon,
                              QWidget *parent = nullptr) const;
...
public:
  ...
  HomeScreenWidgetDesktop(HomeScreenController *controller,
                          QWidget *parent = nullptr);
...
protected:
  ...
  QWidget *makeMenuWidget(HomeScreenController *controller,
                          QWidget *parent = nullptr);

  /** Creates the recent files widget. */
  QWidget *makeRecentFilesWidget(HomeScreenController *controller,
                                 QWidget *parent = nullptr);

  /** Creates the tip-of-the-day widget. */
  QWidget *makeTipsWidget(HomeScreenController *controller,
                          QWidget *parent = nullptr);
...
public:
  ...
  HomeScreenWidgetMobile(HomeScreenController *controller,
                         QWidget *parent = nullptr);
...
public slots:
  ...
  /** Shows the GNSS settings page directly from the field setup action. */
  void showGnssSettings();

  /** Shows the bundled example maps in a focused mobile browser. */
  void showExamples();
...
protected:
  ...
  void addItemToFileList(const QFileInfo &file_info, int hint = 0,
                         const QIcon &icon = {});

  /** Add a single item to the file list, using a custom display name. */
  void addItemToFileList(const QString &label, const QFileInfo &file_info,
                         int hint = 0, const QIcon &icon = {});
