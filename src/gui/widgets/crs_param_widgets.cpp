...
UTMZoneEdit::UTMZoneEdit(CRSParameterWidgetObserver& observer, QWidget* parent)
 ...
 , observer(observer)
{
	auto const zone_expression = QRegularExpression(QStringLiteral("(?:[0-5]?[1-9]|[1-6]0)(?: [NS])?"));
	auto const zone_list = makeZoneList();
	
	line_edit = new QLineEdit();
	line_edit->setValidator(new QRegularExpressionValidator(zone_expression, line_edit));
