...
namespace Ocd
{
	...
	namespace string
	{
		...
		unsigned char assign(const QByteArray& value, unsigned char max_length, char* first, char* last) noexcept
		{
			...
			auto const length = static_cast<unsigned char>(std::min(qsizetype{max_length}, value.length()));
		...
		unsigned char assignUtf8(const QString& value, unsigned char max_length, char* first, char* last)
		{
			...
			auto length = static_cast<unsigned char>(std::min(qsizetype{max_length}, utf8.length()));
