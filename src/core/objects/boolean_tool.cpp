 ...
 */
size_t qHash(const Point64& point, size_t seed)
{
 ...
 * Compares a MapCoord and Clipper2Lib::Point64.
 */
[[maybe_unused]] bool operator==(const Clipper2Lib::Point64& lhs, const MapCoord& rhs)
{
	return rhs == lhs;
}
...
{
	for (size_t i = 0, count = tree.Count(); i < count; ++i)
		outerPolyNodeToPathObjects(*tree[i], out_objects, proto, polymap);
}
...
void outerPolyNodeToPathObjects(const Clipper2Lib::PolyPath64& node, PathObjects& out_objects, const PathObject* proto, const PolyMap& polymap)
{
	...
	try
	{
		...
		for (size_t i = 0, i_count = node.Count(); i < i_count; ++i)
		{
			polygonToPathPart(node[i]->Polygon(), polymap, object.get());
			
			// Add outer polygons contained by (nested within) holes ...
			for (size_t j = 0, j_count = node[i]->Count(); j < j_count; ++j)
				outerPolyNodeToPathObjects(*(*node[i])[j], out_objects, proto, polymap);
		}
