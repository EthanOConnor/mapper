...
project baseline is CMake 4.4.0, Ninja 1.13.0, a C++23 compiler, and Qt 6.12.0.
Desktop and Android packages are built by the same presets used in GitHub
Actions.

Qt 6.12 is required for the Qt Canvas Painter screen renderer. Install the
Canvas Painter module alongside the platform kit; it is part of the supported
runtime and packaging closure, not an optional renderer experiment.
...
Install Qt 6.12.0 with the Canvas Painter, Image Formats, Positioning, Sensors,
and Serial Port modules. Clone and bootstrap the vcpkg baseline recorded in `vcpkg.json`,
...
Qt 6.12.0 `android_arm64_v8a` kit with Canvas Painter. Set `QT_ROOT_DIR`, `VCPKG_ROOT`,
